...
if isnan(s.x)
   ...
   % Prediction according to PI
   state_trans = s.A*s.x;  % State transition matrix update (unity)
   control_inp = s.B*s.u; % Control input matrix update (PI)
   s.xd =  state_trans + control_inp; % A priori estimate (PI update of previous estimate of state)
   s.Pd = s.A * s.P * s.A' + s.Q;
   
   % Keep a track of what pure PI estimate would look like
   s.xPI = s.xPI + control_inp;

   % Compute Kalman gain factor:
   s.K = s.Pd*s.H'/(s.H*s.Pd*s.H'+s.R);

   % Correction based on observation:
   s.x = s.xd + s.K*(s.z-s.H*s.xd);
   s.P = s.Pd - s.K*s.H*s.Pd;
