...
class TDStriatum(object):
    ...
    """
    def __init__(self, env=Environment(), learning_rate=.001, gamma=.98, lamb=.76):
        """
        ...
        self.n_landmark_cells = 100 * 2  # per landmark
        self.n_landmark_centres = 100
        self.indices = [list(range(self.n_landmark_centres)),
                        list(range(self.n_landmark_centres, self.n_landmark_centres * 2))]
        self.landmark_cell_centres = np.linspace(-self.max_viewing_angle, self.max_viewing_angle, self.n_landmark_centres)
    ...
    def compute_LC_activity(self, width=27.5):
        ...
        for i, landmark in enumerate(self.env.objects):
            if self.env.cues_present == 'A':
                indices = self.indices[0]
            elif self.env.cues_present == 'B':
                indices = self.indices[1]
            elif self.env.cues_present == 'both':
            ...
            activity[indices] = (1 / (euclidian_distance + .5)) * np.exp(- (angular_distances ** 2 / (2 * (self.field_width) ** 2)))
