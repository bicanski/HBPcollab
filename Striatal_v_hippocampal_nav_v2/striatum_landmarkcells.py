...
class TDStriatum(object):
    ...
    def compute_LC_activity(self, width=27.5):
        ...
        :return:
        """
        # FIXME: this is a function and procedure in 1.
        ...
        for landmark in self.env.landmark_locations:
            ...
            #activity += (1/ (euclidian_distance+.5)) * np.exp(- (angular_distances ** 2 / (2 * (self.field_width / (euclidian_distance + .2))) ** 2)) # / (euclidian_distance * .2)
...
class Striatum(object):
    ...
    def compute_LC_activity(self, width=27.5):
        ...
        """
        # FIXME: this is a function and procedure in 1.
        # TODO: make this also l2 normalised per row (i.e. per state)
        self.previous_LC_activity = self.LC_activations
        activity = np.zeros(self.LC_activations.shape)
        for landmark in self.env.landmark_locations:
            # First compute the relative angle to the landmark
...
class CognitiveStriatum(object):
    def __init__(self, env=DeterministicTask(), learning_rate=.01, gamma=.98, lamb=.76):
        self.env = env
        self.learning_rate = learning_rate
        self.gamma = gamma
        self.lamb = lamb
        ...
        # Sensory cells
        self.n_landmark_cells = self.env.nr_states
        self.LC_activations = np.zeros(self.n_landmark_cells)
        self.previous_LC_activity = np.zeros(self.n_landmark_cells)

        # Action value cells
        self.n_action_cells = 2
        self.previous_striatum_activation = None
        self.striatum_activation = np.zeros(self.n_action_cells)
        self.weight_mat = np.zeros((self.n_action_cells, self.n_landmark_cells))
        self.eligibility_trace = np.zeros(self.weight_mat.shape)
    ...
    def compute_LC_activity(self):
        self.previous_LC_activity = self.LC_activations
        LC_activations = np.zeros(self.n_landmark_cells)
        LC_activations[self.env.curr_state] = 1.
        return LC_activations
    ...
    def update_trace(self):
        chosen_action = self.env.curr_action_idx
        gen_phase_activity = np.zeros(self.env.nr_actions)
        gen_phase_activity[chosen_action] = 1.
        weight_importance = np.outer(gen_phase_activity, self.LC_activations)
        self.eligibility_trace = weight_importance + self.lamb * self.eligibility_trace
